<template>
  <div>
    ...
    <v-form>
        ...
        <v-btn @click="submit({id,name,email,password})">{{id ? 'Edit' : 'Add'}}</v-btn>
    </v-form>
  </div>
</template>
...
export default {
    ...
    methods:{
        async submit(user){
            if(user.id){
                await this.$axios.post("http://localhost:8081/api/v1/users/"+user.id,{
                    name: user.name,
                    email: user.email,
                    password: user.password
                })
            }else {
                await this.$axios.post("http://localhost:8081/api/v1/users/",{
                    name: user.name,
                    email: user.email,
                    password: user.password
                })
